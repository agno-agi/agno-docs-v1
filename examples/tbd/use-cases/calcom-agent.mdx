...
<Step title="Install libraries">```bash pip install requests pytz ```</Step>

{' '}

<Step title="Set environment variables">
  ```bash export CALCOM_API_KEY="your_api_key" export CALCOM_EVENT_TYPE_ID="your_event_type_id" ```
</Step>

{' '}

<Step title="Run the agent">```bash python calcom_agent.py ```</Step>

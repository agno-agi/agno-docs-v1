...
<Steps>
  <Step title="Install libraries">```bash pip install agno openai exa_py ```</Step>
  <Step title="Set environment variables">```bash export EXA_API_KEY=**** ```</Step>
  <Step title="Run the agent">```bash python study_scout_agent.py ```</Step>
</Steps>

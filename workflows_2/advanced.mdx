...
You can run a workflow in the background by passing `background=True` to `Workflow.arun()`. This will return a `WorkflowRunResponse` object that you can use to get the result of the workflow.

<Note>
This feature is only available for async workflows using `Workflow.arun()`. 
For long-running workflows, you can poll the result using `result = workflow.get_run(run_id)` which returns the current `WorkflowRunResponse`. 

Use `result.has_completed()` to check if the workflow has finished executing. This is particularly useful for workflows that involve time-consuming operations like large-scale data processing, multi-step research tasks, or batch operations that you don't want to block your main application thread.
</Note>

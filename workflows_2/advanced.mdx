...
## Media Input
**Media Artifact Handling in Workflows 2.0**: Workflows seamlessly handle media artifacts (images, videos, audio) throughout the execution pipeline. 
Media can be provided as input in `.run/.print_response` and is passed through to steps via `WorkflowExecutionInput` using `ImageArtifact`, `VideoArtifact`, 
and `AudioArtifact` objects, which are automatically converted to the appropriate `Image`, `Video`, and `Audio` formats when passed to `agents/teams`. 

During execution, media artifacts accumulate across steps - each step receives shared media from previous steps and can 
produce additional media outputs. The Step class handles automatic conversion between artifact formats 
(e.g., `ImageArtifact.content` with base64 data to `Image.content` with actual bytes), ensuring compatibility between 
workflow components and `agent/team` executors. All media artifacts are preserved in `StepOutput` and propagated to 
subsequent steps, creating a comprehensive media flow where the final WorkflowRunResponse contains all accumulated 

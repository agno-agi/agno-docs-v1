...
**<u>StepInput Schema</u>**

The `StepInput` object is passed to each step executor and contains all the data needed for execution:

```python
@dataclass
class StepInput:
    # Primary input message (can be any format)
    message: Optional[Union[str, Dict[str, Any], List[Any], BaseModel]] = None
    
    # Data flow from previous steps
    previous_step_content: Optional[Any] = None  # Content from the last step
    previous_step_outputs: Optional[Dict[str, StepOutput]] = None  # All previous step outputs by name
    
    # Additional context data
    additional_data: Optional[Dict[str, Any]] = None
    
    # Media inputs (accumulated from workflow input and previous steps)
    images: Optional[List[ImageArtifact]] = None
    videos: Optional[List[VideoArtifact]] = None
    audio: Optional[List[AudioArtifact]] = None
    
    # Helper methods
    def get_step_content(step_name: str) -> Optional[Union[str, Dict[str, str]]]
    def get_all_previous_content() -> str
    def get_last_step_content() -> Optional[str]
```

**<u>StepOutput Schema</u>**

The `StepOutput` object is returned by each step executor and contains the execution results:

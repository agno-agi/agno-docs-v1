...
It has the following attributes which you can see in the detailed documentation in the [WorkflowRunResponse](/reference/workflows_2/workflow_run_response) documentation.
...
<Note>
In the case where you don't put `stream_intermediate_steps=True`, we only yield `WorkflowStartedEvent`, `WorkflowCompletedEvent` along with all the `Agent/Team` events.
</Note>
...
The `Workflow.arun()` function is the async version of `Workflow.run()`. It returns an async iterator of `WorkflowRunResponseEvent` objects instead of a single response.

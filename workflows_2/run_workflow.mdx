...
Many of our examples use `workflow.print_response()` which is a helper utility to print the response in the terminal. It uses `workflow.run()` under the hood.
...
It has the following attributes which you can see in the detailed documentation in the [WorkflowRunResponse](/reference/workflows_2/workflow-run-response) documentation.
...
<Note>
In the case where you don't put `stream_intermediate_steps=True`, we only yield `WorkflowStartedEvent` and `WorkflowCompletedEvent`.
</Note>
...
| Event Type | Description |
|------------|-------------|
| `StepsExecutionStarted` | Indicates the start of Steps |
| `StepsExecutionCompleted` | Signals successful completion of Steps |

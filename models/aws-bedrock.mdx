...
<Note>
Async usage of AWS Bedrock is supported. When using `AwsBedrock` with an `Agent`, you can use `agent.arun` and `agent.aprint_response`. 
\nSee [Async Agent with Tools](/examples/models/aws/bedrock/async_tool_use_stream) for an example.
</Note>

...
async def run_server() -> None:
    ...
    async with MCPTools("npx -y @modelcontextprotocol/server-github") as mcp_tools:
        ...
        playground = Playground(agents=[agent], app_id="mcp_demo_app", name="MCP Demo App", description="MCP Demo App").get_app()

        # Serve the app while keeping the MCPTools context manager alive
        playground.serve("github_playground:app", reload=True)

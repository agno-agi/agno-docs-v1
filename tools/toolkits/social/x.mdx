...
| --------------------- | --------------------------------------------------------------------------------------------------- |
| `create_post`         | Creates and posts new content. Returns post URL with confirmation.                                 |
| `reply_to_post`       | Replies to an existing post using post ID. Returns reply URL.                                      |
| `send_dm`             | Sends direct message to user by username or ID. Handles user lookup automatically.                 |
| `get_my_info`         | Retrieves authenticated user's profile details and metrics.                                        |
| `get_user_info`       | Retrieves information about a specific user by username.                                           |
| `get_home_timeline`   | Gets home timeline with configurable number of posts.                                              |
| `search_posts`        | Searches for recent posts based on query with optional metrics.                                    |

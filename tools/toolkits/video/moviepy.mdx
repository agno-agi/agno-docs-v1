...
description: Learn how to use Agno MoviePyVideoTools to process videos, extract audio, generate SRT caption files, and embed rich, word-highlighted captions.
---

**MoviePyVideoTools** provide a suite of functionalities for processing video files within the Agno framework. These tools allow agents to extract audio from videos, create SRT caption files from transcribed text, and embed detailed, word-highlighted captions directly into videos.
...
```
*Please ensure you have a video file (e.g., `video.mp4`) in your working directory or provide a full path for the agent to process it.*

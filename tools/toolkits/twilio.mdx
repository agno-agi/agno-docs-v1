...
| ------------- | --------------- | ------- | ------------------------------------------------- |
| `account_sid` | `Optional[str]` | -  | Twilio Account SID for authentication.            |
| `auth_token`  | `Optional[str]` | -  | Twilio Auth Token for authentication.             |
| `api_key`     | `Optional[str]` | -  | Twilio API Key for alternative authentication.    |
| `api_secret`  | `Optional[str]` | -  | Twilio API Secret for alternative authentication. |
| `region`      | `Optional[str]` | -  | Optional Twilio region (e.g., `au1`).             |
| `edge`        | `Optional[str]` | -  | Optional Twilio edge location (e.g., `sydney`).   |

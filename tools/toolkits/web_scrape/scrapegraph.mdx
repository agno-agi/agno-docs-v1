---
title: ScrapeGraph
description: Learn how to use Agno ScrapeGraphTools to extract structured data from webpages using LLMs or convert webpages to markdown.
---

**ScrapeGraphTools** enable an Agent to interact with the ScrapeGraph library, allowing it to perform intelligent web scraping using Large Language Models (LLMs) to extract structured data, or to convert entire webpages into markdown format.
...
| `markdownify`  | `bool`          | `False` | Enable the `markdownify` tool. Note: If `smartscraper` is explicitly set to `False`, `markdownify` will be automatically enabled (`True`). |

## Developer Resources

- View [Tools Source](https://github.com/agno-agi/agno/blob/main/libs/agno/agno/tools/scrapegraph.py) (Please verify this path)
- View [Cookbook Example](https://github.com/agno-agi/agno/blob/main/cookbook/tools/scrapegraph_tools.py) (Please verify this path) 
...
<Steps>
    ...
    You can use any working MCP server. To see some examples, you can check [this GitHub repository](https://github.com/modelcontextprotocol/servers),
    by the maintainers of the MCP themselves.
    </Step>
...
## Understanding Server Parameters using SSE

If the MCP server you want to connect to uses the SSE transport layer, configure `MCPTools` by using the `transport` (should be set to `"sse"`) and `url` parameters.

You can also use the `server_params` parameter with servers using the SSE transport. In this case, it needs to be an instance of `SSEClientParams`. It contains the following fields:
- `url`: The URL of the MCP server.
- `headers`: Optional headers to pass to the MCP server.
- `timeout`: Optional timeout for the connection to the MCP server.
- `sse_read_timeout`: Optional timeout for the SSE connection itself.

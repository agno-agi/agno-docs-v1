...
By default Agno's MCP integration uses the stdio transport. But you can also connect your Agents to MCP servers using the SSE or Streamable HTTP transports:
...
Now let's set up the same server and agent, but using the Streamable HTTP transport:

...
The KnowledgeTools toolkit can be used with different types of knowledge bases, such as:

- `UrlKnowledge`: Create a knowledge base from web URLs
- `FileKnowledge`: Create a knowledge base from local files
- `SqlKnowledge`: Create a knowledge base from SQL databases
- `CustomKnowledge`: Create a knowledge base from custom data sources

Each knowledge base type can be configured with different vector databases:
...
file_knowledge = FileKnowledge(
    files=["data/document1.pdf", "data/document2.txt"],
    vector_db=ChromaDb(

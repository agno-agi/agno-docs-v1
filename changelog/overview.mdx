...
- **Gemini auto tool calling**: Added auto tool calling when tools are provided, similar to how other models work.
- **FixedSizeChunking issue with overlap:** Fixed issue where chunking would fail if overlap was set.
- **Claude tools with multiple types**: Fixed issue where Claude tools would break for a union of types in the parameters.
- **JSON response parsing**: Fixed cases with JSON model responses where the model provides quoted strings in the dictionary’s values.
</Update>

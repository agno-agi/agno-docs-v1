...
- `MessageMetrics`: The core structure for metrics, attached to each message and tool execution.
...
Here, you can see the metrics for each message sent by the agent. This includes metrics for both assistant messages and tool calls, helping you understand the performance and resource usage at the message level.
...
- When a model or tool call is made, usage data (tokens, time, etc.) is extracted and stored in a MessageMetrics object.
- For tool calls, metrics are attached to the `ToolExecution`.
- For each message, metrics are attached to the `Message`.
- At the end of a run, metrics are aggregated and attached to the `RunResponse`.

...
|-----------|------|---------|-------------|
| `description` | `str` | - | A description of the Agent that is added to the start of the system message. |
| `task` | `str` | - | Describe the task the agent should achieve. |
| `instructions` | `List[str]` | - | List of instructions added to the system prompt in `<instructions>` tags. Default instructions are also created depending on values for `markdown`, `output_model` etc. |
| `additional_context` | `str` | - | Additional context added to the end of the system message. |
| `expected_output` | `str` | - | Provide the expected output from the Agent. This is added to the end of the system message. |
| `extra_instructions` | `List[str]` | - | List of extra instructions added to the default system prompt. Use these when you want to add some extra instructions at the end of the default instructions. |
...
| `system_prompt` | `str` | - | System prompt: provide the system prompt as a string |
...
| `user_prompt` | `Union[List, Dict, str]` | - | Provide the user prompt as a string. Note: this will ignore the message sent to the run function. |
| `user_prompt_template` | `PromptTemplate` | - | Provide the user prompt as a PromptTemplate. |

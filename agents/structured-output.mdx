...
You can enable streaming when setting `response_model`. This doesn't stream the response content itself, but ensures an iterator is still returned, allowing you to stream intermediate steps and processing updates.
...
When using streaming with structured output in teams, the final `content` chunk will contain only one chunk with the complete structured output, rather 
than streaming the content progressively.

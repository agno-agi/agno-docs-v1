...
<Tip>**Sessions** are equivalent to **threads** in the OpenAI Assistant API.</Tip>
...
<Note>
  User preferences and conversation summaries are currently only compatible with `OpenAI` and
  `OpenAILike` models. While Persistent Memory is compatible with all model providers.
</Note>

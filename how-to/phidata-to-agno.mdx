...
The CLI has been completely revamped from `phi` to `ag`. Here are the key changes:
...
phi ws up -> ag ws up
phi ws down -> ag ws down
phi ws patch -> ag ws patch
phi ws restart -> ag ws restart
...
- `phi.aws.resource.xxx` ➔ `agno.aws.resource.xxx`
- `phi.docker.xxx` ➔ `agno.docker.xxx`

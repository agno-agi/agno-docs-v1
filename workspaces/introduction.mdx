...
- A REST API (FastAPI) to serving Agents, Teams and Workflows.
...
The codebases are set up to run locally using Docker and can be deployed to AWS with a single command. They’re a fantastic starting point and exactly what we use for our customers. Years of learning, available for free to the open-source community.

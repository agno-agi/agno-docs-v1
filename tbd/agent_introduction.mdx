...
<Note>
  Under the hood, the description and instructions are converted to the system prompt and the input
  is passed as the user prompt. Use `debug_mode=True` to log the messages sent to the LLM.
</Note>

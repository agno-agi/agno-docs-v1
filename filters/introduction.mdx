...
When configuring your Agent, it's important to choose the right approach for your use case. In traditional RAG, you typically set `add_references=True` to ensure that references are included in the responses. However, in Agentic RAG, you should use `search_knowledge=True` to leverage the agent's ability to infer and apply filters automatically. 

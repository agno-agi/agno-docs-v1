---
title: Migrating to Memory V2
sidebarTitle: Migrating to Memory V2
---

## Migrating to Memory V2

Starting with Agno version 1.4.0, **Memory V2** is now the default memory for the Agno Agent. This replaces the previous `AgentMemory` class which is now deprecated but still available to use.
...
- **Session Summaries:**
  ...
  - **Now:**
    ...
    memory = Memory()
    agent = Agent(create_session_summary=True, memory=memory) or team = Team(create_session_summary=True, memory=memory)
    ```

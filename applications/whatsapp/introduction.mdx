...
The Whatsapp App is used to serve Agents or Teams interacting via WhatsApp, using a FastAPI server to handle webhook events and send messages.
...
### 2. `GET /webhook`
    ...
    *   **Query Parameters**:
        ...
        | ----------------- | ----- | ------------------------------ |
        | `hub.mode`        | `str` | Expected: "subscribe".         |
        | `hub.verify_token`| `str` | Your verification token.       |
        | `hub.challenge`   | `str` | Challenge from WhatsApp.       |
    *   **Responses**:
...
### 3. `POST /webhook`
    ...
    *   **Headers**:
        | Header                | Description                                |
        | --------------------- | ------------------------------------------ |
        | `X-Hub-Signature-256` | SHA256 signature for payload validation. |
    *   **Request Body**: JSON payload from WhatsApp with message details.
    *   **Processing**:

...
Storage is a necessary component when building user-facing AI products. While agents come with built-in memory, it only lasts during the active session. For persistent conversations across sessions, Agno provides storage options that work with both individual agents and complex workflows.
...
Agno supports the following databases as storage backends: PostgreSQL, Sqlite, SingleStore, DynamoDB, and MongoDB.
